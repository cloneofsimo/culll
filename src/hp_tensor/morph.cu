...
__global__ void expMaximumSwapKernel(lint* batched_data_a_man,
                                     lint* batched_data_b_man,
                                     int* batched_data_a_expo,
                                     int* batched_data_b_expo,
                                     lint* batched_data_a_sign,
                                     lint* batched_data_b_sign, int B, int N,
                                     int M, int n) {
...
void HPTensor::swap_exp_maximum(HPTensor* other) {
  ...
  expMaximumSwapKernel<<<dimGrid, dimBlock>>>(
      this->mantissa, other->mantissa, this->exponent, other->exponent,
      this->sign, other->sign, B, N, M, n);
}

__global__ void shiftExponentKernel(int* batched_data_expo, int* amount, lint B,
                                    lint N, lint M) {
  int batch_idx = blockIdx.x * blockDim.x + threadIdx.x;
  int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
  int col_idx = blockIdx.z * blockDim.z + threadIdx.z;
...
__global__ void shiftMantissaKernel(lint* batched_data_man, int* amount, lint B,
                                    lint N, lint M, lint n, lint logbase) {
  int batch_idx = blockIdx.x * blockDim.x + threadIdx.x;
  int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
  int col_idx = blockIdx.z * blockDim.z + threadIdx.z;
    ...
    if (is_leftshift) {
      for (int i = n - shiftamount_blk - 1; i >= 1; i--) {
        batched_data_man[pos2 + shiftamount_blk + i] =
            ...
            (batched_data_man[pos2 + i - 1] >> (logbase - shiftamount_rem));
      }
...
void HPTensor::reposition(int* amount) {
  ...
  // shift mantissa
  shiftMantissaKernel<<<dimGrid, dimBlock>>>(this->mantissa, amount, B, N, M,
                                             precision, logbase);
