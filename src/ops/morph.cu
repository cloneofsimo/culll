...
__global__ void batchBigTensorKernelZeroPad(lint *batched_data_a, lint *output,
                                            lint B, lint N, lint M, lint n1,
                                            lint n2, lint base) {
  int batch_idx = blockIdx.x * blockDim.x + threadIdx.x;
  int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
  int col_idx = blockIdx.z * blockDim.z + threadIdx.z;
...
void batchBigTensorNegateWrapper(pybind11::array_t<lint> batched_data_a,
  ...
  batchBigTensorKernelNegate<<<dimGrid, dimBlock>>>(gpu_ptr_a, B, N, M, n,
                                                    base);
...
void batchBigTensorDigitResizeWrapper(pybind11::array_t<lint> batched_data_a,
  ...
  batchBigTensorKernelDigitResize<<<dimGrid, dimBlock>>>(gpu_ptr_a, gpu_ptr_b,
                                                         B, N, M, n1, n2, base);
