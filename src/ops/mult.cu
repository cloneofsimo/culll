...
__global__ void batchLongTensorMult(
...
void batchLongTensorMultWrapper(
    pybind11::array_t<lint> batched_data_a, pybind11::array_t<lint> batched_data_b, pybind11::array_t<lint> output_data, int mode = 1, int verbose = 0, int base = 10)
{
    ...
    dim3 dimGrid(B, N, M);
    if (mode == 0) {
        batchLongTensorMult << <dimGrid, dimBlock >> > (gpu_ptr_a, gpu_ptr_b, gpu_ptr_c, B, N, M, n, base);
    }
    else {
